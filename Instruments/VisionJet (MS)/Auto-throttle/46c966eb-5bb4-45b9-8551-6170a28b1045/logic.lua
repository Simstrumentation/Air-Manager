...
Auto throttle for the Vision Jet by MS/Asobo
...
- Will only work with the MS Vision Jet. Compatibility with other aircraft not 
...
--THROTTLE
local settings = { { 0 , -16383},
                   { 1, 16383 }}
                   
function throttle_position(pos)

    sliderVal = interpolate_linear(settings, 1 - pos)

    msfs_event("AXIS_THROTTLE_SET", sliderVal)
...
msfs_variable_subscribe("L:XMLVAR_SpeedIsManuallySet", "enum",
                                              "B:SF50_AUTOPILOT_AUTOTHROTTLE_ARM", "ENUM",
                                              setFMSAn)
