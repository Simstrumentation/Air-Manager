...
-- Adjustable values
--local calculate_period = 500 -- milliseconds between recalculating output Lux value
...
local lux_scale_params =   	{ {150000  , 1      }, -- 
				{  75000   , 0.8    }, -- 
				{  4000    , 0.6    }, -- 
				{  1000    , 0.40   }, -- 
				{  700     , 0.25    }, -- 
...
local rotation_speed = 360/365.25
...
function calculate()
	...
	local sin_lat = math.sin(math.rad(latitude))
	...
	local earth_latitude_radius = earth_radius * cos_lat -- ths radius of the slice of earth along the latitude line
	...
	local declination = math.deg(math.asin(0.3978 * math.sin(math.rad(rotation_speed) *(daynum - (81 - 2 * math.sin((math.rad(rotation_speed) * (daynum - 2))))))))
	local sin_declination = 0.3978 * math.sin(math.rad(rotation_speed) *(daynum - (81 - 2 * math.sin((math.rad(rotation_speed) * (daynum - 2))))))
	local hourlyAngle = 15 * ( 12 - (decimalTime + (4 * longitude / 60 )) ) 
	...
	local sunAltitude_corrected = sunAltitude + relative_horizon
	local sinSunAltitude = math.sin(math.rad(sunAltitude_corrected))
	local M = (math.sqrt(1229 + (614 * sinSunAltitude)^2) - 614 * sinSunAltitude) * press_rel/press_abs
	...
	elseif (sunAltitude_corrected) < -7 then  -- it's dark
        ...
        print(okta)    
	-- update output value
	ready_for_output = (1- (interpolate_linear(lux_scale_params, math.abs(var_round(weightedLux,2))))) 
end
...
function update_callback(var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10, var_11, var_12)
        ...
        decimalTime  = var_3 + (offset_minutes/60)
        visibility = var_4
        if var_4 < 50000 then -- hack to handle visibility only working with live weather, not presets.  Live weather seems to give a value of 50000m or less
