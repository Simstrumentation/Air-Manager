...
----------------------------------------------
--Wing
msfs_variable_subscribe("L:ASCRJ_AICE_WING", "Number", 
        function (state)
...
sw_wing= switch_add(nil,nil, 74,76,181,179,
    function (position)
        if (position == 1 ) then
            msfs_variable_write("L:ASCRJ_AICE_WING","Number",0) 
        elseif (position == 0 ) then
            msfs_variable_write("L:ASCRJ_AICE_WING","Number",1) 
        end
    end)

--Cowl L
msfs_variable_subscribe("L:ASCRJ_AICE_COWL_L", "Number", 
        function (state)
...
sw_cowl_L= switch_add(nil,nil, 242,76,181,179,
    function (position)
        if (position == 1 ) then
            msfs_variable_write("L:ASCRJ_AICE_COWL_L","Number",0) 
        elseif (position == 0 ) then
            msfs_variable_write("L:ASCRJ_AICE_COWL_L","Number",1) 
        end
    end)

--Cowl R
msfs_variable_subscribe("L:ASCRJ_AICE_COWL_R", "Number", 
        function (state)
...
sw_cowl_R= switch_add(nil,nil, 366,76,181,179,
    function (position)
        if (position == 1 ) then
            msfs_variable_write("L:ASCRJ_AICE_COWL_R","Number",0) 
        elseif (position == 0 ) then
            msfs_variable_write("L:ASCRJ_AICE_COWL_R","Number",1) 
        end
    end)

img_ice_det_on = img_add("light_ice.png", 526,104,86,85)  visible(img_ice_det_on, false)
msfs_variable_subscribe("L:ASCRJ_AICE_DETECT_ICE", "Number", 
...
function cb_ice_det_test()
    msfs_variable_write("L:ASCRJ_AICE_DETECT","Number",1)
    sound_play(click_snd)
    timer_start(50, function() msfs_variable_write("L:ASCRJ_AICE_DETECT","Number",0)end)   
end
...
function cb_wshld_L_dec()
    if current_pos_wshld_L == 0  then msfs_variable_write("L:ASCRJ_AICE_WSHLD_L", "Number", 1) end
    if current_pos_wshld_L == 1  then msfs_variable_write("L:ASCRJ_AICE_WSHLD_L", "Number", 2) end   
end
btn_wshld_L_dn = button_add(nil, nil, 90, 345, 65, 80, cb_wshld_L_dec)

function cb_wshld_L_inc()
    if current_pos_wshld_L == 2  then msfs_variable_write("L:ASCRJ_AICE_WSHLD_L", "Number", 1) end
    if current_pos_wshld_L == 1  then msfs_variable_write("L:ASCRJ_AICE_WSHLD_L", "Number", 0)  end
end
btn_wshld_L_up = button_add(nil, nil, 90, 235, 65, 80, cb_wshld_L_inc)

msfs_variable_subscribe("L:ASCRJ_AICE_WSHLD_L", "Number", 
    function (pos)
        if pos == 0 then
...
sw_wshld_test= button_add(nil,"button_pressed.png", 137,268,158,115,
    function ()
            msfs_variable_write("L:ASCRJ_AICE_WSHLD_TEST","Number",1) 
    end)
    
--- WSHLD R
function cb_wshld_R_dec()
    if current_pos_wshld_R == 0  then msfs_variable_write("L:ASCRJ_AICE_WSHLD_R", "Number", 1) end
    if current_pos_wshld_R == 1  then msfs_variable_write("L:ASCRJ_AICE_WSHLD_R", "Number", 2) end   
end
btn_wshld_R_dn = button_add(nil, nil, 290, 345, 65, 80, cb_wshld_R_dec)

function cb_wshld_R_inc()
    if current_pos_wshld_R == 2  then msfs_variable_write("L:ASCRJ_AICE_WSHLD_R", "Number", 1) end
    if current_pos_wshld_R == 1  then msfs_variable_write("L:ASCRJ_AICE_WSHLD_R", "Number", 0)  end
end
btn_wshld_R_up = button_add(nil, nil, 290, 235, 65, 70, cb_wshld_R_inc)

msfs_variable_subscribe("L:ASCRJ_AICE_WSHLD_R", "Number", 
    function (pos)
        if pos == 0 then
...
------------------------------------------------------------------------------
--Probes L
msfs_variable_subscribe("L:ASCRJ_AICE_PHEAT_L", "Number", 
        function (state)
...
sw_pheat_L= switch_add(nil,nil, 364,251,181,179,
    function (position)
        if (position == 1 ) then
            msfs_variable_write("L:ASCRJ_AICE_PHEAT_L","Number",0) 
        elseif (position == 0 ) then
            msfs_variable_write("L:ASCRJ_AICE_PHEAT_L","Number",1) 
        end
    end)

--Probes R
msfs_variable_subscribe("L:ASCRJ_AICE_PHEAT_R", "Number", 
        function (state)
...
sw_pheat_R= switch_add(nil,nil, 489,251,181,179,
    function (position)
        if (position == 1 ) then
            msfs_variable_write("L:ASCRJ_AICE_PHEAT_R","Number",0) 
        elseif (position == 0 ) then
            msfs_variable_write("L:ASCRJ_AICE_PHEAT_R","Number",1) 
        end
    end)
